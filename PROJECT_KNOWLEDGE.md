...
- Kamera **powoli dryfuje** (`CAMERA_DRIFT`, `CAMERA_RATE`) i to ona zamienia istniejącą w tej scenie głębię w coś widocznego. Nieruchoma kamera renderuje scenę 3D i płaski obraz w identyczne piksele — samo wybrzuszenie czyta się jako gradient, nie jako przestrzeń. Płaty stoją bardzo nierówno (4, 8,5 i 20 jednostek), więc jedno przesunięcie kamery przemieszcza najbliższy o 8,5% połowy kadru, a najdalszy o 1,7% — ta różnica JEST sygnałem głębi. Dryf kosztuje czapę: sprawdzenie pokrycia trzeba prowadzić z krańcowych położeń kamery, nie ze środka.
...
- Środkowa warstwa oddycha: rozjaśnia się i przygasa **cosinusem**, nie piłą. Wcześniej była to pięćdziesięciosekundowa rampa zerowana skokiem — stąd nagły przeskok tła do stanu początkowego przy każdym obiegu. Rampa szła też do 100, co mnożyło każdy kanał tak daleko poza biel, że kolor znikał na długo przed szczytem; `PULSE_MAX` (20) to świadome prześwietlenie — powyżej ok. 1,9 typowa barwa nicku ma wszystkie kanały ponad 1 i szczyt oddechu jest biały. Cosinus odpowiada za płynność wzrostu i opadania, a nie za ograniczenie jasności. `PULSE_MIN` decyduje o tym, jak ciemne wydaje się tło, bo tam oddech spędza połowę czasu — podnoszenie `PULSE_MAX` nic na to nie daje.
- Grafiki warstw: **PNG z kanałem alfa, 16:9** (proporcję czapy ustala `TEXTURE_ASPECT` — inna proporcja pliku wyjdzie rozciągnięta), mapowane raz na czapę, więc nie muszą być kafelkowalne. Kolejność w `LAYER_TEXTURES` jest myląca: `warstwa3` jest najdalej, `warstwa1` w środku (i to ona pulsuje jasnością), `warstwa2` najbliżej gracza. Na ekranie widać 57–81% szerokości pliku zależnie od warstwy, więc przy 3840×2160 teksele są powiększone 1,2–1,8×; najbardziej przednia warstwa.
- Muszą mieć **mipmapy** (`mipmaps/generate=true`). Były importowane jako 2D bez nich, co po przeniesieniu na zakrzywione powierzchnie dawało aliasing na obrzeżach, gdzie tekstura jest pomniejszana.
- Wrażenie, że warstwy „przebijają się" przez siebie, ma DWIE osobne przyczyny i warto je rozróżniać. Pierwsza to geometria (niżej). Druga, znacznie bardziej widoczna, to falowanie przezroczystości środkowej warstwy: przy 2,0 rad/s wahało się ono 0,60–1,00 **co trzy sekundy**, na przemian zasłaniając i odsłaniając warstwę za sobą. Było to jedyne szybkie zjawisko w całym tle — wszystko inne cykluje w 18–217 s — więc spowalnianie jasności nic na nie nie dawało.

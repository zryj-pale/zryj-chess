...
- Pozycje menu głównego to wyciągnięte w 3D napisy z `assets/NAPISY 3D/` wiszące w powietrzu przed animowanym tłem, a nie zwykłe `Button`. Kolumna stoi po lewej stronie ekranu (`SIGN_ALIGN_X`), barwi się kolorem nicku (tym samym co tło), a każdy napis kołysze się i **obraca się wokół własnej osi** — pełny obrót zajmuje ok. 16 s (`SPIN_RATE`).
- Najechanie myszą zatrzymuje obrót i obraca napis przodem do gracza (`lerp_angle` idzie krótszą drogą), do tego podjeżdża on w stronę kamery i rozjaśnia się. To zatrzymanie jest tym, co czyni obracające się menu używalnym: pozycja, na którą wskazujesz, jest tą, którą da się przeczytać.
- Trafienia liczone są ręcznie — prostokąt napisu jest rzutowany na ekran przez `unproject_position()`, ale **celowo bez obrotu**: obracające się słowo bywa ustawione bokiem i pole kliknięcia zwężone razem z nim byłoby wtedy praktycznie nie do trafienia. Prostokąt spoczynkowy zawsze pokrywa to, co narysowane, więc błąd idzie w stronę wybaczającą.

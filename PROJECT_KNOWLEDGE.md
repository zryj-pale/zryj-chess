...
- W tle menu pływają dodatkowo prawdziwe `BoardTile` — te same płytki, z których zbudowana jest plansza — dryfując powoli przed wszystkimi trzema płatami i zawijając się na krawędziach ekranu. Włączane flagą `floating_tiles` ustawianą między `instantiate()` a `add_child()`; w meczu zostają wyłączone, żeby nie mylić się z planszą.
- Całe tło przechodzi przez ten sam shader zakłócający co napisy menu, ale na ok. jednej piątej siły (`FX_*`). Musi objąć całe tło, a nie same kafelki: kafelki dzielą viewport z płatami grafiki i są z nimi przeplecione w głąb, czego osobny viewport by nie odtworzył. W praktyce nic to nie kosztuje, bo płaty nie mają twardych krawędzi, o które siatka mogłaby zaczepić — efekt widać wyłącznie na kafelkach. Rozmiar bloku jest ten sam co na napisach (żeby obie siatki się zgadzały), a zjeżdża `softness` i wszystkie zakłócenia; `tint` zostaje biały, bo niebieski z napisów przebarwiłby całe tło.
- Kafelki dryfują **między** warstwami tła (głębokość 10–21, gdy przednia czapa jest na 4,0, środkowa na 8,5, a tylna na 16), więc przednia grafika przechodzi po nich, a one zasłaniają dalsze. To działa dlatego, że czapy testują głębię, ale jej nie zapisują.
- Kafelki są rozstawiane **po** dopasowaniu viewportu do okna, nie przed. `_visible_half()` czyta proporcję z viewportu, a ten do czasu dopasowania ma domyślny kwadratowy rozmiar — przez to płytki lądowały w kwadratowym polu i pokrywały pełną wysokość, ale tylko środkową połowę szerokości (zmierzone: x 0,24–0,76 przy y 0,02–0,98).
- Rozstawione po jednym na komórkę siatki 5×4 z rozrzutem wewnątrz, mierzonym na EKRANIE a nie w jednostkach świata — czternaście niezależnych losowań kłębiło się w jednym rogu. Orientacje są w pełni losowe i każdy obraca się wokół własnej stałej osi, więc płytki bywają ustawione krawędzią.
- Kafelki pożyczają rig oświetleniowy `BoardTile`, ale ten jest dobrany do planszy kilku jednostek tuż pod kamerą. Tutaj pole ma dwadzieścia jednostek głębi, więc lampa jest przewieszona i wszystkie światła podkręcone — przy nastawach planszy kafelki wypadały poza zasięg lampy i robiły się płaskie. Ruszane są WYŁĄCZNIE światła: `BoardTile` trzyma materiały w statycznym cache wspólnym z prawdziwą planszą.
- Każdy kafelek wisi w węźle-uchwycie, bo `BoardTile` co klatkę nadpisuje własną rotację (swoją lewitacją). Uchwyt niesie obrót stawiający płytkę przodem do kamery, powolny obrót w płaszczyźnie ekranu i dryf; płytka w środku dalej sobie faluje.
- Czapy tła mają **włączony test głębi** (a kolejność między nimi trzyma `render_priority`). Wyłączenie testu sprawiłoby, że zamalowywałyby wszystko, co jest przed nimi — w tym pływające kafelki.

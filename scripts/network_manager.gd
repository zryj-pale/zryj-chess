...
signal remote_ready(pieces: Array, card: String)
signal game_started(white_pieces: Array, black_pieces: Array, coin: String, turn: String, white_card: String, black_card: String)
...
func send_ready(pieces: Array, card: String) -> void:
	_send_reliable({"type": "ready", "pieces": pieces, "card": card if CardRegistry.is_valid(card) else ""})

func start_game(host_white: Array, guest_black: Array, host_card: String, guest_card: String) -> void:
	if not is_host:
		return
	...
	_send_reliable({"type": "start", "white": white_pieces, "black": black_pieces, "coin": coin_result, "turn": initial_turn, "white_card": white_card, "black_card": black_card})
	game_started.emit(white_pieces, black_pieces, coin_result, initial_turn, white_card, black_card)
	...
	match payload.get("type", ""):
		"ready":
			if payload.get("pieces", []) is Array:
				remote_ready.emit(payload["pieces"], str(payload.get("card", "")))
		"start":
